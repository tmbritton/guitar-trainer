...
- [ ] **Story 6.6 — Audio-device selection.** Enumerate devices; bind the duplex device's capture + playback to chosen devices (needed for the input-only Rocksmith Real Tone cable → speakers); persist the choice; picker in Settings.
- [ ] **Story 6.7 — Fast-follow.** A-B loop; drag-drop import; dry monitoring.
